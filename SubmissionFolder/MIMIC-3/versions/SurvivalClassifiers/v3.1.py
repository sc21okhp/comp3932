...
def prepForTraining(train):
    Y_train_val, X_train_val = getXY(train)

    X_train, X_val, Y_train, Y_val = train_test_split(X_train_val, Y_train_val, test_size=0.25, random_state=42)

    #Now we can combine validation set with training set for later
    X_train_full = np.concatenate((X_train, X_val))
    Y_train_full = np.concatenate((Y_train, Y_val))
    test_fold = np.array([-1] * len(X_train) + [0] * len(X_val))

    #Predefined split let's us use val set in GridSearchCV later
    ps = PredefinedSplit(test_fold)

    return X_train_full, Y_train_full, ps

def cIndexScore(model, X_test, Y_test):
...
def trainCox(train):
    X_train_full, Y_train_full, ps = prepForTraining(train)

    initial_grid_search = GridSearchCV(CoxPHSurvivalAnalysis(), 
                                       {}, 
                                       cv=ps, 
                                       scoring=cIndexScore, 
                                       n_jobs=-1)

    initial_grid_search.fit(X_train_full, Y_train_full)

    new_model = initial_grid_search.best_estimator_
    new_params = initial_grid_search.best_params_
    #Needs a weird formatting :(((
    wrapped_params = {param: [value] for param, value in new_params.items()}

    final_grid_search = GridSearchCV(new_model, 
                                     wrapped_params, 
                                     cv=5, 
                                     scoring=cIndexScore, 
                                     n_jobs=-1)
    ...
    return final_grid_search.best_estimator_

def trainElasticCox(train):
    X_train_full, Y_train_full, ps = prepForTraining(train)

    initial_grid_search = GridSearchCV(CoxnetSurvivalAnalysis(), {
                                       'n_alphas': [50, 100],
...
def trainRSF(train):
    ...
    initial_grid_search = GridSearchCV(RandomSurvivalForest(), {
...
def pointBrier(threshold_test_surv_probs, Y_point_test):
    #Death prob = 1 - survival prob
    threshold_test_death_probs = 1.0 - threshold_test_surv_probs

    brier = brier_score_loss(Y_point_test, threshold_test_death_probs)

    print(f'Point Brier Score: {brier}')
...
def evaluate_survival_model(model, model_name, train, test):
    ...
    pointBrier(threshold_test_surv_probs, Y_point_test)
