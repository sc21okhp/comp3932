...
def labelEvent(row):
    ...
    if not row['censored']:
        event = 0
        if row['duration'] <= death_threshold_year:
            event = 1

    #Not dead (Right censored)
    else:
        event  = 0
...
def processPatients(patients, admissions, diagnoses):
    ...
    #Coerce means NaT for invalid
    df['ADMITTIME'] = pd.to_datetime(df['ADMITTIME'], errors='coerce')
    df['DOD'] = pd.to_datetime(df['DOD'], errors='coerce')

    #Stupid calculation took me way too long
    #Because you need every admission per SUBJECT_ID
    df['last_admit'] = df.groupby('SUBJECT_ID')['ADMITTIME'].transform('max')

    #Make sure death can't be before admission
    #(Except where death is more than a day before admission)
    #(I'm looking at you 10518)
    df['DOD_plus_24h'] = df['DOD'] + timedelta(days=1)

    #31557600
    convert_to_years = 60 * 60 * 24 * 365.25

    #I should've just hard coded this with a for loop
    #Too bad! Now we're here
    df['duration'] = np.where(
        df['EXPIRE_FLAG'] == 1,
        #If they're dead
        (df['DOD_plus_24h'] - df['ADMITTIME']).dt.total_seconds() / convert_to_years,
        #Right censored (Alive)
        (df['last_admit'] - df['ADMITTIME']).dt.total_seconds() / convert_to_years
    )
    #Deal with problems where duration is negative
    #Negative ones seem to be people admitted to hospital after death
    #So I guess we just give them 0 duration???
    df['duration'] = df['duration'].clip(lower=0)

    #Sets it as a bool for later
    ...
    df['event'] = df.apply(labelEvent, axis=1)
...
def trainCox(train):
    print("Started CPH Training")
    Y_train, X_train = getXY(train)

    cph = CoxPHSurvivalAnalysis()
    cph.fit(X_train, Y_train)
    print("Finished CPH Training")
    print()
    return cph
