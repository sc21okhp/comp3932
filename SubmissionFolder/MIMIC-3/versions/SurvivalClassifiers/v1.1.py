...
def calcHazard(features, patient_id):
    ...
    #TODO: Use event_col and include non dead records
    ...
    #If you put a stupid subject id in that's on you
    ...
    risk_score = cph.predict_partial_hazard(patient_features.drop(columns=['SUBJECT_ID'])).values[0]
    #Work out survival probabilities for chosen patient after one year 
    survival_prob_one_year = cph.predict_survival_function(patient_features.drop(columns=['SUBJECT_ID']), times=[1]).iloc[0, 0]
